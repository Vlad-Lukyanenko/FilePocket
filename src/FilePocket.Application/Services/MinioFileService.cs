...
namespace FilePocket.Application.Services
{
        ...
        IMinioService minioService) : IFileService
    {
        ...
        public async Task<NoteModel> GetNoteByUserIdAndIdAsync(Guid userId, Guid fileId)
        {
            ...
            note.Content = await encryptionService.DecryptContent(note.UserId, note.PocketId, fileBytes);
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            async Task UploadFileToDisk(FileMetadata metadata)
            {
        ...
        private string SelectFileDirectory(Guid userId, Guid? pocketId, FileTypes fileType)
        {
            ...
            var path = pocketId is not null
                ? $"{minioService.BucketName}/{userId}/{pocketId}/{now.Year}/{now.Month}/{fileType}s"
                : $"{minioService.BucketName}/{userId}/{now.Year}/{now.Month}/{fileType}s";
